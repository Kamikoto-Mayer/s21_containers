...
TEST(map, MapAtOperatorException) {
  s21::map<char, std::string> my_map = {
      {'a', "Alina"}, {'b', "Boris"}, {'c', "Chuck"}};
  std::map<char, std::string> my_map_std = {
      {'a', "Alina"}, {'b', "Boris"}, {'c', "Chuck"}};

  EXPECT_THROW(my_map_std.at('h'), std::out_of_range);
  EXPECT_THROW(my_map.at('h'), std::out_of_range);
}
...
TEST(map, SwapMap) {
  ...
  EXPECT_EQ(my_map.size(), size2);
  EXPECT_EQ(my_swap_map.size(), size1);
...
TEST(map, ContainsMap) {
  s21::map<int, int> my_map{{1, 1}, {2, 2}, {5, 5}};
  s21::map<std::string, int> my_map2{{"mary", 1}, {"rachel", 2}, {"bob", 5}};
  EXPECT_TRUE(my_map.contains(5));
  EXPECT_FALSE(my_map.contains(6));

  EXPECT_TRUE(my_map2.contains("bob"));
  EXPECT_FALSE(my_map2.contains("john"));
}

TEST(map, InsertManyMap) {
  s21::map<int, int> my_map{{1, 1}, {2, 2}, {5, 5}};
  s21::map<std::string, int> my_map2{{"mary", 1}, {"rachel", 2}, {"bob", 5}};
  my_map.insert_many(std::pair<int, int>(5, 5), std::pair<int, int>(8, 8));
  EXPECT_TRUE(my_map.contains(5));
  EXPECT_FALSE(my_map.contains(6));
